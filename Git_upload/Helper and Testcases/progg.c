...
// Add these logging functions to your progg.c file

void log_elevator_state(Elevator elevators[], int N, int turn, int K) {
    printf("TURN_START:%d\n", turn);
    
    for(int i = 0; i < N; i++) {
        printf("ELEVATOR:%d,%d,%c,%d,%d\n", 
               i, 
               elevators[i].current_floor, 
...
// Add this in your main loop, right after Next_Inst():
// log_elevator_state(elevators, N, turn_counter, K);
// log_pickup_drop(pickcnt, drpcnt, turn_counter);
    ...
    while(!isFinished){
      ...
      //  printf("drp done\n");
       ...
       // printf("rider pick done\n");
        ...
        log_pickup_drop(pickcnt, drpcnt, x);
        for(int i=0;i<N;i++) {
        ...
        printf("\nnumber of total request fulfilled %d and total request are %d",num_of_reqfulfilled,Total_req);
