...
// Global variables for dynamic request handling
FILE* dynamicRequestsFile = NULL;
long lastFilePosition = 0;
int globalRequestId = 0;
#define MAX_DYNAMIC_REQUESTS 10000
PassengerRequestInfo allPassengerRequests[MAX_DYNAMIC_REQUESTS];
int totalRequests = 0;

// Function to read new requests from dynamic file
int readNewDynamicRequests(int currentTurn) {
    if (dynamicRequestsFile == NULL) {
        // Try to open the dynamic requests file
        dynamicRequestsFile = fopen("dynamic_requests.txt", "r");
        if (dynamicRequestsFile == NULL) {
            // File doesn't exist yet, create it
            dynamicRequestsFile = fopen("dynamic_requests.txt", "w");
            if (dynamicRequestsFile != NULL) {
                fclose(dynamicRequestsFile);
                dynamicRequestsFile = fopen("dynamic_requests.txt", "r");
            }
            if (dynamicRequestsFile == NULL) {
                return 0; // No new requests
            }
        }
        lastFilePosition = 0;
    }
    ...
    fseek(dynamicRequestsFile, lastFilePosition, SEEK_SET);
    
    int newRequestsCount = 0;
    int startFloor, requestedFloor, arrivalTime;
    
    // Read new lines from file
    while (fscanf(dynamicRequestsFile, "%d %d %d", &startFloor, &requestedFloor, &arrivalTime) == 3) {
        if (totalRequests < MAX_DYNAMIC_REQUESTS) {
            // Store the new request
            allPassengerRequests[totalRequests].currentLocation = startFloor;
            allPassengerRequests[totalRequests].granted = 0;
            allPassengerRequests[totalRequests].isInElevator = 0;
            allPassengerRequests[totalRequests].movedThisTurn = 0;
            allPassengerRequests[totalRequests].arrivalTime = arrivalTime;
            allPassengerRequests[totalRequests].request.requestedFloor = requestedFloor;
            allPassengerRequests[totalRequests].request.startFloor = startFloor;
            allPassengerRequests[totalRequests].request.requestId = globalRequestId++;
    ...
    lastFilePosition = ftell(dynamicRequestsFile);
    
    printf("Turn %d: Read %d new dynamic requests\n", currentTurn, newRequestsCount);
...
int main(int argc, char* argv[]) {
  ...
  // Initialize variables for dynamic request handling
  int startFloor, requestedFloor, arrivalTime;
  
  // Initialize dynamic requests (no longer need passengerRequestCount from testcase)
  totalRequests = 0;
  globalRequestId = 0;
  ...
  // Main Logic
  int requestsRemaining = 0, errorOccured = 0, totalElevatorMovement = 0;
  ...
  // Dynamic simulation loop - continues until manually stopped or error
  while (!errorOccured) {
    ...
    // Read new dynamic requests from file
    readNewDynamicRequests(turnNumber);

    // DEBUG
    printf("Starting turn number %d\n", turnNumber);

    // Add any new passenger requests for this turn in shared memory
    for (int i = 0; i < totalRequests; i++) {
      if (allPassengerRequests[i].arrivalTime == turnNumber && 
          !allPassengerRequests[i].granted && 
          !allPassengerRequests[i].isInElevator &&
          allPassengerRequests[i].currentLocation != -1) {
        
        if (turnChangeResponse.newPassengerRequestCount < MAX_NEW_REQUESTS) {
          mainShmPtr->newPassengerRequests[turnChangeResponse.newPassengerRequestCount] =
              allPassengerRequests[i].request;
    ...
    requestsRemaining = 0;
    for (int i = 0; i < totalRequests; i++) {
      if (!allPassengerRequests[i].granted) {
        requestsRemaining++;
      }
    }
    ...
    for (int i = 0; i < totalRequests; i++) {
      allPassengerRequests[i].movedThisTurn = 0;
    }
    ...
    for (int i = 0; i < turnChangeRequest.droppedPassengersCount; i++) {
      ...
      // Find the passenger request by ID
      int passengerIndex = -1;
      for (int j = 0; j < totalRequests; j++) {
        if (allPassengerRequests[j].request.requestId == requestId) {
          passengerIndex = j;
          break;
        }
      }
      ...
      if (passengerIndex == -1) {
        printf("Turn %d: Attempted to move unknown passenger %d\n", turnNumber, requestId);
        errorOccured = 1;
        break;
      }

      // Check for errors
      if (allPassengerRequests[passengerIndex].granted) {
        printf(
      ...
      if (allPassengerRequests[passengerIndex].arrivalTime > turnNumber) {
        printf(
            "Turn %d: Attempted to move passenger %d even though their request "
            "has not arrived yet\n",
            turnNumber, requestId);
      ...
      if (!allPassengerRequests[passengerIndex].isInElevator) {
        printf(
      ...
      if (allPassengerRequests[passengerIndex].movedThisTurn) {
        printf(
      ...
      int elevatorNumber = allPassengerRequests[passengerIndex].currentLocation;

      // Update state as requested
      allPassengerRequests[passengerIndex].isInElevator = 0;
      allPassengerRequests[passengerIndex].currentLocation =
          elevatorInfo[elevatorNumber].currentFloor;
      allPassengerRequests[passengerIndex].movedThisTurn = 1;

      // Check if the request has been fulfilled
      if (allPassengerRequests[passengerIndex].request.requestedFloor ==
          allPassengerRequests[passengerIndex].currentLocation) {
        allPassengerRequests[passengerIndex].granted = 1;
        requestsRemaining--;
      }
    ...
    for (int i = 0; i < turnChangeRequest.pickedUpPassengersCount; i++) {
      ...
      if (allPassengerRequests[passengerIndex].currentLocation !=
          elevatorInfo[elevatorNumber].currentFloor) {
        printf(
      ...
      // Update state as requested
      allPassengerRequests[passengerIndex].isInElevator = 1;
      allPassengerRequests[passengerIndex].currentLocation = elevatorNumber;
      allPassengerRequests[passengerIndex].movedThisTurn = 1;
    ...
    // Check if simulation should continue - break if no remaining requests and no activity for several turns
    if (requestsRemaining == 0 && turnChangeResponse.newPassengerRequestCount == 0) {
      printf("Turn %d: No active requests, simulation continues...\n", turnNumber);
      // You can add a timeout here if needed, or let it run indefinitely
    }
  }
