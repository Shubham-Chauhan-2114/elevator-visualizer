...
void SetAuthString(int keys[],int N,int M,Elevator elevators[],MainSharedMemory* shmPtr,int isfree[]){
    ...
    sem_init(&solver_semaphore, 0, M);
    pthread_t threads[N];
    thread_args_t thread_args[N];
    int x = 0;
    for (int i = 0; i < N; i++) {

        sem_wait(&solver_semaphore);
        ...
        thread_args[i].key = keys[x%M];
        thread_args[i].isfree = isfree;
        thread_args[i].elv = elevators[i];
        thread_args[i].kid = x%M;
        thread_args[i].elv_no = i;
        thread_args[i].shmPtr = shmPtr;
        int result = pthread_create(&threads[i], NULL, thread_func, (void*)&thread_args[i]);        
        if (result != 0) {
...
int cost(Rider R, Elevator elevator,int M){
    ...
    for(int i=0 ; i<K_MAX ; i++){
        if(elevator.drop_path[i]>0){
            cnt -= abs(i - elevator.current_floor);
        }
    }
...
// Modified logging functions to write to file instead of stdout
void log_elevator_state(Elevator elevators[], int N, int turn, int K) {
    if (output_file == NULL) return;
    
    fprintf(output_file, "TURN_START:%d\n", turn);
    
    for(int i = 0; i < N; i++) {
        fprintf(output_file, "ELEVATOR:%d,%d,%c,%d,%d\n", 
               i, 
               elevators[i].current_floor, 
        ...
        // Log current riders
        for(int j = 0; j < elevators[i].load; j++) {
            fprintf(output_file, "RIDER_IN_ELV:%d,%d,%d,%d\n", 
                   i,
                   elevators[i].curr_riders[j].requestId,
                   elevators[i].curr_riders[j].startFloor,
                   elevators[i].curr_riders[j].requestedFloor);
        }
        
        // Log waiting riders
        for(int j = 0; j < elevators[i].num_riders_inq; j++) {
            fprintf(output_file, "RIDER_WAITING:%d,%d,%d,%d\n", 
                   i,
                   elevators[i].riders_inq[j].requestId,
                   elevators[i].riders_inq[j].startFloor,
                   elevators[i].riders_inq[j].requestedFloor);
        }
    }
...
void log_pickup_drop(int pickup_count, int drop_count, int turn) {
    if (output_file == NULL) return;
    
    fprintf(output_file, "STATS:%d,%d,%d,%d\n", turn, pickup_count, drop_count, num_of_reqfulfilled);
    fprintf(output_file, "TURN_END:%d\n\n", turn);
    fflush(output_file);
}
...
int main(){
   ...
   // printf("\n");
    ...
    while(!isFinished){
      ...
      //  printf("drp done\n");
       ...
       // printf("rider pick done\n");
        ...
        total_picked += pickcnt;
        total_dropped += drpcnt;
        log_elevator_state(elevators, N, x, K);
        log_pickup_drop(total_picked, total_dropped, x);
        for(int i=0;i<N;i++) {
        ...
        // Write final statistics to file instead of printf
        //fprintf(output_file, "\nnumber of total request fulfilled %d and total request are %d\n", num_of_reqfulfilled, Total_req);
        fflush(output_file);
    ...
    //fprintf(output_file, "\nFinal Summary:\n");
    //fprintf(output_file, "Parameters: N=%d K=%d M=%d T=%d\n", N, K, M, turn);
    //fprintf(output_file, "Total requests fulfilled: %d\n", num_of_reqfulfilled);
    //fprintf(output_file, "Total requests received: %d\n", Total_req);
    
    // Close the output file
    fclose(output_file);
    
    //printf("\n %d %d %d %d %d %d \n ",N,K,M,turn,shm_key,main_msgq_key);
